...
<pre class="Agda">{% raw %}<a id="1355" class="Comment">-- Node : Set</a>
<a id="1369" class="Comment">-- Edge : (x : C) → (P x → Node) → Set</a>

<a id="1409" class="Comment">-- data Graph₁ : Set where</a>
<a id="1436" class="Comment">--  ?</a>{% endraw %}</pre>

1.2. Option B

<pre class="Agda">{% raw %}<a id="1482" class="Comment">-- Node : Set</a>
<a id="1496" class="Comment">-- Edge : Node → Node → Set</a>
<a id="1524" class="Comment">-- data Graph₂ : Set where</a>
<a id="1551" class="Comment">--  α : ∀ {x y : Node} → Edge x y ≃ Edge y x</a>
<a id="1596" class="Comment">--  -- α x x == id?</a>{% endraw %}</pre>

Now we choose which definition we use:

<pre class="Agda">{% raw %}<a id="1681" class="Comment">-- Graph = Graph₂</a>{% endraw %}</pre>
