...
- `T1: Path (coe α a) b`
\begin{code}
T1 : ∀ {ℓ} (A : Type ℓ) → (B : Type ℓ) (α : A == B)(a : A)(b : B) → Type ℓ
T1 A B α a b = Path (coe α a) b

HEq-to-T1 : ∀ {ℓ} (A : Type ℓ) → (B : Type ℓ) (α : A == B)(a : A)(b : B)
          → HEq A B α a b → T1 A B α a b
HEq-to-T1 A B idp a .a hid = idp

T1-to-HEq : ∀ {ℓ} (A : Type ℓ) → (B : Type ℓ) (α : A == B)(a : A)(b : B)
          → T1 A B α a b → HEq A B α a b
T1-to-HEq A B idp a .a idp = hid

-- T1-≃-HEq : ∀ {ℓ} (A : Type ℓ) → (B : Type ℓ) (α : A == B)(a : A)(b : B)
--     → T1 A B α a b ≃ HEq A B α a b
-- T1-≃-HEq A .A idp a b = ( T1-to-HEq A A idp a b , isequiv)
--   where
--     isequiv : isEquiv (T1-to-HEq A A idp a b)
--     isequiv hid = ( ( idp , idp) , λ x → {!  !})
...
\begin{code}
T2 : ∀ {ℓ} {A : Type ℓ}{B : Type ℓ} {α : A == B}{a : A}{b : B} → Type ℓ
T2 {A = A}{B}{α}{a}{b} = Path a (coe (inv α) b)
\end{code}

- `T3: HEq'` defined by path induction (pattern matching in Agda)

\begin{code}
T3 : ∀ {ℓ} {A : Type ℓ}{B : Type ℓ} {α : A == B}{a : A}{b : B} → Type ℓ
T3 {A = A}{B}{idp}{a}{b} = Path a b

-- ≅-to-≡ : ∀ {ℓ} {A : Set ℓ} {B : Set ℓ} {α : A == B}{a : A}{b : B} → HEq A B α a b → a == b
-- ≅-to-≡ = {!   !}
\end{code}
...
data PathOver₁ {ℓ} {A : Set ℓ} (C : A → Type ℓ) {a₁ : A} :
  ...
  -- idp represents reflexivity over reflexivity
\end{code}
...
- \begin{code}
-- pathover defined by path induction
